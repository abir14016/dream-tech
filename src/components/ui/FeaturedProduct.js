...
const FeaturedProduct = ({ featuredProduct }) => {
    ...
    return (
        <div class="p-4">
            <a href="" class="c-card block bg-base-200 shadow-md hover:shadow-xl rounded-lg overflow-hidden">
                <div class="relative pb-48 overflow-hidden">
                    <img class="absolute inset-0 h-full w-full object-cover" src={image} alt="" />
                </div>
                <div class="p-4">
                    <span class="inline-block px-2 py-1 leading-none bg-warning text-orange-800 rounded-full font-semibold uppercase tracking-wide text-xs">{category}</span>
                    <h2 class="mt-2 mb-2  font-bold">{name}</h2>
                    <div class="mt-3 flex items-center">
                        <span class="text-sm font-semibold"></span>&nbsp;<span class="font-bold text-xl">{price}</span>&nbsp;<span class="text-xl font-semibold">$</span>
                    </div>
                </div>
